...
module.exports.sockets = {
  ...
  onConnect: function(session, socket) {
    ...
    if(session.user != null){
        ...
        clearTimeout(this.logout[session.user.idSession]);
    }
  ...
  onDisconnect: function(session, socket) {
    // MODIFICAR EL DESTROY A TRUE EN SESSIONS
    if(session.user != null){
        
        this.logout[session.user.idSession] = setTimeout(function(){
            
            Sessions.update({id: session.user.idSession},{destroy: true}).exec(function(err,user){
